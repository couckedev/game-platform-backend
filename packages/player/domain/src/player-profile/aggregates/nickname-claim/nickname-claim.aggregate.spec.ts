...
describe("Nickname claim aggregate", () => {
  describe("claim", () => {
    it("should produce nickname claimed domain event", () => {
      ...
      const nicknameClaimedEvent = new NicknameClaimedEvent(
        nickname,
        claimer,
        now,
      );
      expect(nicknameClaim.getUncommittedEvents()).toContainEqual(
        nicknameClaimedEvent,
      );
    });
  ...
  describe("aggregateId", () => {
    it("should use nickname value as aggregate id", () => {
      const nickname = Nickname.create("nickname");

      const nicknameClaim = new NicknameClaim(nickname);

      expect(nicknameClaim.aggregateId).toStrictEqual(nickname.value);
    });
  });
});
