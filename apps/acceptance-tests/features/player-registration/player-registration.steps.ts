...
import { ExternalAccountId, Nickname, NicknameRejectionReason, Player, PlayerRejectionReason, PlayerId } from "player-domain";
...
Given<GamePlatformWorld>(
  ...
  async function (nickname: string) {
    ...
    const externalAccountId = ExternalAccountId.create(this.requireExternalAccountId());
    const nicknameVO = Nickname.create(nickname);
    const createdAt = this.requireClock().now();
    const existingPlayer = Player.register(playerId, nicknameVO, externalAccountId, createdAt);
...
Given<GamePlatformWorld>(
  "external account id is already used by another player",
  async function () {
    const playerId = PlayerId.create("other-player-id");
    const externalAccountId = ExternalAccountId.create(this.requireExternalAccountId());
    const nicknameVO = Nickname.create("other_nick");
    const createdAt = this.requireClock().now();
    const existingPlayer = Player.register(playerId, nicknameVO, externalAccountId, createdAt);
    const playerRepository = this.requirePlayerRepository();
    await playerRepository.save(existingPlayer);
...
When<GamePlatformWorld>("registration is requested", async function () {
  try {
    const registerPlayerInput: RegisterPlayerInput = {
      ...
      externalAccountId: this.requireExternalAccountId()
    };
...
Then<GamePlatformWorld>("player is online", async function () {
  const playerId = PlayerId.create(this.requirePlayerId());
  const player = await this.requirePlayerRepository().load(playerId);
  assert.ok(player, "Expected player to exist");
...
Then<GamePlatformWorld>(
  "registration is rejected because external account id is already used",
  function () {
    assert.equal(
      this.registrationError?.code,
      PlayerRejectionReason.ExternalAccountIdAlreadyUsed,
    );
  },
);
